...
def distance_squared(c1, c2):
	return (c1.x_coord - c2.x_coord)**2 + (c1.y_coord - c2.y_coord)**2
...
def distanceBetween(NodeA, NodeB):
	...
	return int(round(math.sqrt(((NodeB.getXCoordinate() - NodeA.getXCoordinate()) ** 2) 
...
def main():
	...
	start = default_timer()
	tour, d = TSP_NN(listOfNodes)
	solutionList = performSimulatedAnnealing(tour, 200000, 40000, .99)
	end = default_timer()
